...
class CarController extends ApiBaseController
{
    ...
    private function getKeySpecifications(Car $car)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $list[$i]['title'] = $key;
            if (($spec->input_type) == CarAdditonalSpecifications::TYPE_BOOLEAN) {
                $list[$i]['value'] = $value == 1 ? 'Yes' : 'No';
            } else
                $list[$i]['value'] = $value;
                ...
                'features' => [
                    ...
                    ($varient->power || $varient->torque) ? [
                        'title' => ($varient->power && $varient->torque) ? 'Power & Torque'
                            : ($varient->power ? 'Power' : 'Torque'),
                        'value' => trim(
                            ($varient->power ? $varient->power . 'hp' : '') .
                            ($varient->power && $varient->torque ? ' and ' : '') .
                            ($varient->torque ? $varient->torque . ' nm' : '')
                        ),
                    ] : null,
