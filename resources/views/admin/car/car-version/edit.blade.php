...
<x-card title="Update Car Version">
    <x-form method="PUT" action="{{ route('admin.car-version.update', $carVarient) }}" >
        <div class="row">
            <div class="col-md-12 col-sm-12 col-12">
                <div class="row">
                    ...
                    <!-- <div class="col-md-4">
                        ...
                        <span class="error" role="alert" id="car_id_error" ></span>
                    </div> -->
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <x-form-select field="status" field-name="Status*" defaultPrompt="Select status">
                            @foreach (config('params.car.status') as $value => $label)
                                <option {{ $carVarient->status == $value ? 'Selected' : '' }} value="{{ $value }}">
                                    {{ $label }}</option>
                            @endforeach
                        </x-form-select>
                    </div>
        ...
        <div class="card card-border card-primary">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <x-form-input type="text" field="ex_showroom_price" field-name="Ex-Showroom Price*" value="{{  $carVarient->ex_showroom_price }}">
                        </x-form-input>
                    </div>
                    <div class="col-md-4">
        ...
        <div class="card card-border card-primary">
            ...
            <div class="card-body">
                <div class="col-md-12">
                    <label class="control-label" for="profession">Colors</label>
                    <div class="row">
                            @php
                                $allColorCheck = '';
                                if ($ccount == $selectedColorsCount) {
                                    $allColorCheck = 'checked';
                                }
                            @endphp
                        ...
                        <div class="form-check form-check-inline col-md-2">
                                    <input class="form-check-input" type="checkbox" name="colors[]"
                                        id="colors{{ $key }}" value="{{ $key }}" {{ $attrCCheck }}>
                                    &nbsp;<label class="form-check-label" for="colors{{ $key }}"
                                        style="color: black;">
                                        {{ $value }}
                                    </label>
                        </div>  
                    ...
                    <div class="col-md-4">
                        <x-form-select field="transmission_type" field-name="Transmission Type*" defaultPrompt="Select transmission type">
                            @foreach (config('params.car.transmission_type') as $value => $label)
                                <option {{ $carVarient->transmission_type == $value ? 'Selected' : '' }} value="{{ $value }}">
                                    {{ $label }}</option>
                            @endforeach
                        </x-form-select>
                    </div>
        ...
        <script type="text/javascript">
            ...
            function selectAllColors()
            {
                if ($("#all_colors").prop("checked")) {
                    ...
                    var count = document.getElementById("ccount").value;
                    for(var i = 1; i <= count; i++) {
                        $("#colors"+i).attr("disabled", false);
                        $("#colors"+i).prop("checked", false);
                    }
                }
            }
            ...
            if('{!! $currentCar !!}') {
                ...
                const carOption = new Option(currentCar.text, currentCar.id, true, true);
