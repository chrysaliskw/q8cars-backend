<x-admin-layout title="Video Stories">
    ...
    {{-- <div class="row">
        ...
        @php 
            $count = $count;
        @endphp
    <x-crud-create title="Video Stories">
        <x-form method="POST" action="{{ route('admin.news.video.store') }}" class="form" enctype="multipart/form-data">
            <div class="row">
                ...
                <div class="col-md-4">
                    <x-form-select field="language" field-name="Language" defaultPrompt="Select language">
                        @foreach(config('params.news.posts.language') as $value => $label)
                            <option {{ old("language") == $value ? "Selected" : "" }} value="{{ $value }}">{{ $label }}</option>
                        @endforeach
                    </x-form-select>
                </div>
            </div>
            ...
            <div class="row">
                <input type="hidden" name="count" id="count" value="{{$count}}" />
                <div class="col-md-4">
                    <x-form-input type="file" field="image" field-name="Video Thumbnail" value="{{ old('image') }}"></x-form-input>
                </div>
                <div class="col-md-4">
                    <x-form-input type="text" field="youtube_video_link" field-name="Youtube Video Link" value="{{ old('video_link') }}"></x-form-input>
                </div>
                    ...
                    <div class="row"> 
                        <div class="form-check form-check-inline col-md-4">
                            <input class="form-check-input" type="checkbox" name="all_countries" 
                                id="all_countries" value="-1" onclick="selectAllCountries()">
